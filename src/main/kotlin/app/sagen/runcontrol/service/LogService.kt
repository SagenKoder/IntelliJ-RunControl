...
class LogService(private val project: Project) {
    ...
    fun listLogSources(configName: String): List<LogSource> {
        ...
        val descriptor = ExecutionManager.getInstance(project)
    ...
    fun readConsoleOutput(
        ...
        val descriptor = ExecutionManager.getInstance(project)
            .getRunningDescriptors { true }
            .find { it.displayName == configName } ?: return null

        // Try to get console view from attached content
        val consoleView = descriptor.attachedContent?.component as? ConsoleView ?: return null

        // Try to get text from console
        // Different console implementations may require different approaches
        val text = try {
    ...
    fun searchConsole(
        ...
        val descriptor = ExecutionManager.getInstance(project)
            .getRunningDescriptors { true }
            .find { it.displayName == configName } ?: return emptyList()

        val consoleView = descriptor.attachedContent?.component as? ConsoleView ?: return emptyList()
        val text = try {
            getConsoleText(consoleView)
        } catch (e: Exception) {
            return emptyList()
     ...
     */
    private fun getContext(lines: List<String>, index: Int, contextSize: Int): List<String> {
    ...
    private fun getConsoleText(consoleView: ConsoleView): String {
        return try {
            ...
            val editor = (consoleView as? Any)?.let { console ->
                // Use reflection to access editor if available
                val editorField = console.javaClass.declaredFields
     ...
     */
    fun tailConsole(configName: String, lines: Int = 100): LogContent? {
     ...
     */
    fun tailLogFile(filePath: String, lines: Int = 100): LogContent {
        val file = File(filePath)
        if (!file.exists()) {
